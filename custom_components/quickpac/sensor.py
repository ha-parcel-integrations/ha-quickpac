"""Sensor platform for the Quickpac parcel tracker integration."""
...
from .coordinator import QuickpacCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: QuickpacConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Quickpac sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        QuickpacIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        QuickpacAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            QuickpacParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(QuickpacNextDeliverySensor(coordinator, entry))
    entities.append(QuickpacDeliveredParcelsSensor(coordinator, entry))
    entities.append(QuickpacLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class QuickpacIncomingParcelsSensor(
    CoordinatorEntity[QuickpacCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                QuickpacParcelSensor(self.coordinator, self._entry, barcode)
...
class QuickpacAwaitingPickupSensor(
    CoordinatorEntity[QuickpacCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: QuickpacCoordinator, entry: ConfigEntry) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class QuickpacParcelSensor(CoordinatorEntity[QuickpacCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Quickpac parcel."""
    ...
    def __init__(
        self, coordinator: QuickpacCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class QuickpacNextDeliverySensor(
    CoordinatorEntity[QuickpacCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: QuickpacCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class QuickpacDeliveredParcelsSensor(
    CoordinatorEntity[QuickpacCoordinator], SensorEntity
):
    """Recently delivered tracked Quickpac parcels."""
...
class QuickpacLastUpdateSensor(
    CoordinatorEntity[QuickpacCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Quickpac was last polled successfully."""
