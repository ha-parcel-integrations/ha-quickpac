"""Coordinator for the Quickpac parcel tracker integration.
...
from .api import QuickpacApiClient, QuickpacApiError
from .const import (
...
class QuickpacCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (QuickpacApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Quickpac fetch failed for %s: %s", code, result)
        ...
        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Quickpac unreachable for all tracked parcels")
