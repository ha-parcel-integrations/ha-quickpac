"""Tests for the Quickpac coordinator: fetching, caching, events and the
...
from custom_components.quickpac.api import QuickpacApiError
...
from custom_components.quickpac.coordinator import (
    QuickpacCoordinator,
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["LastStatusCode"] = 1401
    sample["LastStatus"] = "In Bearbeitung"
    return sample
...
async def test_polling_is_hot_for_an_out_for_delivery_parcel(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = active_sample()  # In Zustellung
    coordinator = QuickpacCoordinator(hass, client, entry)
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = QuickpacApiError("HTTP 500")
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = QuickpacCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"IdentCode": "GONE"}
...
async def test_delivery_time_changed_never_fires_no_eta_field_exists(hass):
    """Quickpac's public response has no ETA field anywhere — planned_from/to
    are always None, so this event can never fire. Documents the gap rather
    than testing a mechanism that has nothing to drive it."""
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = QuickpacCoordinator(hass, client, entry)
