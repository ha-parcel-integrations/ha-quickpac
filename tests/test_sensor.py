"""Tests for Quickpac sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.quickpac.const import ParcelStatus
from custom_components.quickpac.sensor import (
    QuickpacAwaitingPickupSensor,
    QuickpacDeliveredParcelsSensor,
    QuickpacIncomingParcelsSensor,
    QuickpacLastUpdateSensor,
    QuickpacNextDeliverySensor,
    QuickpacParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = QuickpacIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = QuickpacParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = QuickpacParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = QuickpacNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = QuickpacNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = QuickpacDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = QuickpacLastUpdateSensor(coordinator, _entry())
