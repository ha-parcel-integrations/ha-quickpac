...
when nothing is tracked or everything tracked is delivered. Since Quickpac's
public path never populates `planned_from` (see "Deliberate `None`s and
other decisions" above), an `out_for_delivery` parcel always goes straight to
the hot tier — there is no lookahead window to wait for. See
